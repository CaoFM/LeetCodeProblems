...
If you add a plus ('+') in the local name, everything after the first plus sign will be ignored. This allows certain emails to be filtered, for example m.y+name@email.com will be forwarded to my@email.com.
...
class Solution:
    def numUniqueEmails(self, emails) -> int:
        new_emails =[]
        for item in emails:
            new_emails.append(self.cleanEmailAddress(item))

        return len(set(new_emails))

    def cleanEmailAddress(self, address):
...
if __name__ == '__main__':
    ...
    s = Solution()
    print(s.numUniqueEmails(input_list))